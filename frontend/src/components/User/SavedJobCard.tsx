...
import { fetchSavedJobs, setSavedCurrentJob } from "../../slices/jobSlice";
import { useAppDispatch } from "../../redux/store";

export default function SavedJobCard({ job }: { job: Job }) {
    const navigate = useNavigate();
    const [cookies] = useCookies(["token", "user"]);
    const [bookMark, setBookMark] = useState<"saved" | "notSaved">("saved");
    ...
    function getItem(id: number) {
        dispatch(setSavedCurrentJob(id))
    }
    ...
    return (
        <>
            <li key={job.id} className={`${theme} rounded-md p-2 cursor-pointer`}>
                ...
                <section className="space-y-2" onClick={() => getItem(job.id)}>
